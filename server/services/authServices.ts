...
export async function check_user_existence(_username: string, _email:string){
    const username_is_existing: boolean = Boolean(await prisma.user.findUnique({ where: { email: _email } }));
    const mail_is_existing: boolean = Boolean(await prisma.user.findUnique({ where: { username: _username } }));

    if (username_is_existing || mail_is_existing) {
        const errors: { username?: string; email?: string } = {};

        if (username_is_existing) errors.username = "This username is already taken.";
...
export async function create_user(username: string, email:string, password:string): Promise<User>{
